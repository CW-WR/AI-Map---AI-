...
async def convert_docx(
    ...
    if not MAMMOTH_AVAILABLE:
        try:
            ...
            raise HTTPException(500, "DOCX 转换功能暂时不可用（mammoth 和 LibreOffice 都未安装）")
...
async def convert_pdf(
    ...
    if not FITZ_AVAILABLE:
        raise HTTPException(500, "PDF 转换功能暂时不可用（PyMuPDF 未安装）")
...
async def convert_pptx(
    ...
    if not FITZ_AVAILABLE:
        raise HTTPException(500, "PPTX 转换功能暂时不可用（PyMuPDF 未安装）")
